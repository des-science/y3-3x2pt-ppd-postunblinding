...
export RUNNAME=3x2pt_lcdm_SR_maglim_boosted
...
cd ..
srun -n ${SLURM_NTASKS} cosmosis --mpi ppd/maglim_3x2_goodnessoffit/ppd.ini -p listppd.size_chunk=1000 listppd.i_chunk=${SLURM_ARRAY_TASK_ID} output.filename=ppd/maglim_3x2_goodnessoffit/ppd_chain_${RUNNAME}_maglim_3x2_goodnessoffit_${SLURM_ARRAY_TASK_ID}.txt
cd ppd
