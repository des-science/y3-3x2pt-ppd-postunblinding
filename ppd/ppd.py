...
def execute(block, config):
    ...
    if (config['condition_on_d']):
        ...
        dprime_realization_all = np.random.multivariate_normal(mean_dprime_conditioned, cov_dprime_conditioned, size=ndraws)
        dprime_realization = dprime_realization_all[0]
    else:
        mean_dprime = theory[dprime_indices]
        # for _ in range(ndraws):
        #     dprime_realization = np.random.multivariate_normal(mean_dprime, cov_dprime)
        #     dprime_realization_all.append(dprime_realization)
        dprime_realization_all = np.random.multivariate_normal(mean_dprime, cov_dprime, size=ndraws)
        dprime_realization = dprime_realization_all[0]
    ...
    if (statistic == 'chi2'):
        ...
        #         c += 1.
        # c /= ndraws
        diff_realization = dprime_realization_all - dprime_theory[None,:]
        chi2_realization = np.diagonal(np.dot(diff_realization, np.dot(inv_cov_dprime_chi2, diff_realization.T)))
        c = np.sum((chi2_realization>chi2_data).astype(float)) / ndraws
        ...
        for like_name in ['xip', 'xim', '1x2', 'gammat', 'wtheta', '2x2']:
            ...
            if len(_like_indices)>0:
                ...
                #         _c += 1.
                # _c /= ndraws
                _r = dprime_realization_all[:,_like_indices]
                _chi2_r = np.diagonal(np.dot(_r-_th[None,:], np.dot(_icov, (_r-_th[None,:]).T)))
                _c = np.sum((_chi2_r>_chi2_d).astype(float)) / ndraws

                block['ppd', 'chi2_'+like_name+'_comp_avg'] = _c
                # block['ppd', 'chi2_'+like_name+'_realization'] = _chi2_r
                block['ppd', 'chi2_'+like_name+'_realization'] = _chi2_r[0]
            else:
