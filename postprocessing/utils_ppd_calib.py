...
def dot(A,B):
    if np.iscomplexobj(A) or np.iscomplexobj(B):
        return _dot(A.astype(np.complex128), B.astype(np.complex128))
    else:
        return _dot(A,B)
...
def symmatrix_sqrt_inv(A):
    ...
    #return np.linalg.inv(np.linalg.cholesky(cov_nearest(A)))
...
def get_chi2_rsd_sub(p_U_means, p_U_dvs):
    ...
    for i in numba.prange(n):
        w_i = np.zeros(m)
        for j in range(m):
            w_i[j] = np.sum(np.square(np.abs(p_U_means[i]-p_U_dvs[j])))
        w[i] = w_i
    return w
...
def get_chi2_one_sub(p_U_means, p_U_dvs):
    ...
    w = np.zeros(n)
    for i in numba.prange(n):
        w[i] = np.sum(np.square(np.abs(p_U_means[i]-p_U_dvs[i])))
    return w
