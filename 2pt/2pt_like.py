...
class TwoPointLikelihood(GaussianLikelihood):
    ...
    def extract_theory_points(self, block):
        ...
        for spectrum in self.two_point_data.spectra:
            ...
            dataset_names = dataset_names + spectrum.name + ','
            # dataset_names.append(np.repeat(spectrum.name, len(bin1_vector)))
            si = self.all_names.index(spectrum.name)
            dataset_indices.append(np.repeat(si, len(bin1_vector)))
        ...
        block[names.data_vector, self.like_name + "_dataset_names"] = dataset_names
        block[names.data_vector, self.like_name + "_dataset_indices"] = dataset_indices
